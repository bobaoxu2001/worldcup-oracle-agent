...
async function main() {
  ...
  const trueMax = ALL_MATCH_RESULTS.reduce<string | null>(
  ...
  check("getRecordedResultCount equals the recorded array length", getRecordedResultCount() === ALL_MATCH_RESULTS.length, `${getRecordedResultCount()}`);
  ...
  // 3. getDataFreshnessSummary aggregates honestly.
  ...
  check("summary liveCache.ageHours is null or a number", summary.liveCache.ageHours === null || typeof summary.liveCache.ageHours === "number");
