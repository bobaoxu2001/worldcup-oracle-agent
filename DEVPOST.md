...
- **Through 11 July (100 completed matches, quarter-finals included):** the live engine calls the right result **68% of the time (68/100)**, scores **+44% skill on the Ranked Probability Score** (the football-forecasting standard) vs a no-information coin-flip, and is **well-calibrated — ECE 4.8%** (when it says 70%, it happens ~70% of the time). It went 4-for-4 on the quarter-finals.
...
| **Verified accuracy** | Walk-forward backtest of the live engine on every completed 2026 result (`/accuracy`, `GET /api/accuracy`): **68% top-pick, +44% RPS skill, ECE 4.8%**, cross-checked by an independent Dixon-Coles LOO fit. Reproduced by `npm run backtest`; guarded by `npm run test:track`. |
| **Demo video** | _paste your 3-min link_ |
...
5. **Open `/accuracy` (Track Record).** "This isn't a demo that only looks smart — here's the model graded against the real tournament, walk-forward. 68% of results called right, +44% skill over a coin-flip, well-calibrated — and an independent Dixon-Coles model agrees. Every call is on the record, misses included."
