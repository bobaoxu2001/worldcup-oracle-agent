...
def load_fixtures(data_dir, name_to_idx):
    """Load upcoming fixtures (no goals) to forecast; skip any with an unknown
    team (a side that didn't play in the fitted data)."""
    path = os.path.join(data_dir, "fixtures.csv")
    if not os.path.exists(path):
        return []
    out = []
    for r in schema.load_csv(path):
        ...
        out.append({
            ...
            "round": r.get("round", ""),
            "home": h, "away": a,
            "hi": name_to_idx[h], "ai": name_to_idx[a],
            "home_slot": r.get("home_slot", ""), "away_slot": r.get("away_slot", ""),
            "home_flag": 1 if r.get("neutral_or_home_context", "neutral").strip() == "home" else 0,
        })
    return out


def forecast_fixtures(params, fixtures):
    """Full-data-fit forecast for each fixture: 1X2 + most-likely scorelines."""
    res = []
    for fx in fixtures:
        mat = score_matrix(params, fx["hi"], fx["ai"], fx["home_flag"])
        ph, pd_, pa = outcome_probs(mat)
        pick, conf = max((("home", ph), ("draw", pd_), ("away", pa)), key=lambda kv: kv[1])
        verdict = fx["home"] if pick == "home" else fx["away"] if pick == "away" else "Draw"
        res.append({**fx, "ph": ph, "pd": pd_, "pa": pa, "top": top_scorelines(mat, 3),
                    "pick": pick, "conf": conf, "verdict": verdict})
    return res


def forecast_markdown(forecasts, today):
    lines = [f"# Ridge Dixon-Coles — Round-of-32 forecast (as of {today})", ""]
    lines.append("_Auto-generated by `dc_model.py` from the full-data fit. These are")
    lines.append("genuine forward forecasts — the R32 has not been played._")
    lines.append("")
    lines.append("> Note: the **most-likely single scoreline** is often a low draw (1-1 / 0-0)")
    lines.append("> even when a team is the 1X2 favourite — win probability is spread across")
    lines.append("> many scorelines while one exact score sits highest. Read the 1X2 columns")
    lines.append("> for who is favoured; the scorelines for the modal results.")
    lines.append("")
    lines.append("| Tie | Home win | Draw | Away win | Most likely | Top scorelines |")
    lines.append("|---|---|---|---|---|---|")
    for f in forecasts:
        lines.append(
            f"| {f['id']} {f['home']} v {f['away']} "
            f"| {f['ph']*100:.0f}% | {f['pd']*100:.0f}% | {f['pa']*100:.0f}% "
            f"| **{f['verdict']}** ({f['conf']*100:.0f}%) | {', '.join(f['top'])} |"
        )
    return "\n".join(lines) + "\n"
...
def run(data_dir, write_outputs=True):
    ...
    # Forward forecast for any resolved upcoming fixtures (e.g. the Round of 32).
    name_to_idx = {teams[i]: i for i in range(len(teams))}
    fixtures = load_fixtures(data_dir, name_to_idx)
    forecasts = forecast_fixtures(full_params, fixtures) if fixtures else []
    if forecasts:
        print("\n-- ROUND-OF-32 FORECAST (full-data fit; not yet played) ------------")
        for f in forecasts:
            print(f"  {f['id']:<5} {f['home']:<22} v {f['away']:<22}  "
                  f"{f['verdict']} {f['conf']*100:.0f}%   (H {f['ph']*100:.0f}/D {f['pd']*100:.0f}/A {f['pa']*100:.0f}, ML {f['top'][0]})")
    ...
    if write_outputs:
        ...
        if forecasts:
            with open(os.path.join(reports_dir, "dc_r32_forecast.md"), "w", encoding="utf-8") as f:
                f.write(forecast_markdown(forecasts, today))
            wrote += " + reports/dc_r32_forecast.md"
        print("\nWrote " + wrote)
...
def selftest():
    ...
    # Forecast path: probs sum to 1, verdict is one of the two teams or Draw.
    fx = [{"id": "x", "round": "R", "home": teams[1], "away": teams[4],
           "hi": 1, "ai": 4, "home_slot": "", "away_slot": "", "home_flag": 0}]
    fc = forecast_fixtures(full, fx)[0]
    check("fixture forecast probs sum to 1", abs(fc["ph"] + fc["pd"] + fc["pa"] - 1.0) < 1e-6)
    check("fixture verdict is a valid label", fc["verdict"] in (teams[1], teams[4], "Draw"))
    # Home advantage raises the home team's win prob.
    fc_home = forecast_fixtures(full, [{**fx[0], "home_flag": 1}])[0]
    check("home advantage raises home win prob", fc_home["ph"] > fc["ph"])
